...
jest.mock('./sendEmail/SendEmailTo', () => {
  const {View} = require('react-native');
  return jest.fn(() => <View testID="send-email-to" />);
});

jest.mock('./sendEmail/EmailOptions', () => {
  const {View} = require('react-native');
...
jest.mock('./QPBottomSheet', () => {
  const {View} = require('react-native');
  return jest.fn(({children, visible}) =>
    visible ? <View testID="qp-bottom-sheet">{children}</View> : null,
  );
});

jest.mock('./QPBottomSheetHeader', () => {
  const {View} = require('react-native');
  return jest.fn(() => <View testID="qp-bottom-sheet-header" />);
});
