...
const FilterHeader = props => {
  ...
  let endDate = moment(props.range.endDate, DMYFORMAT).format(
    HalfMonthDateYearFormat,
  );
  ...
  let reduceRange = () => {
    ...
    let tempEnd = moment([endComponents[2], endMonth + '', endComponents[0]]);
    let days = tempEnd.diff(tempStart, 'days');
    let endDay = moment(startDate, DMYFORMAT)
      .subtract(1, 'days')
      .format(DMYFORMAT);
    let startDay = moment(endDay, DMYFORMAT)
      .subtract(days, 'days')
      .format(DMYFORMAT);
...
export const styles = StyleSheet.create({
  filterHeader: {
    ...
    paddingLeft: DeviceInfo.isTablet()
      ? 1.6 * PaddingConstants.tab1
      : 1.1 * PaddingConstants.tab2,
    alignItems: 'center',
