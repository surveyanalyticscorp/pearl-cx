...
import AsyncStorage from '@react-native-async-storage/async-storage';
import useForgotPasswordProcess, {
  getFormatedEmailAndAccessCode,
  isValidInput,
} from './useForgotPasswordProcess';
import {requestPasswordLink} from '../../../../redux/actions/login.actions';
import {setUserDetailsForResetPassword} from '../../../../redux/actions';
...
jest.mock('../../../../redux/actions', () => ({
  setUserDetailsForResetPassword: jest.fn(),
}));

describe('useForgotPasswordProcess Hook', () => {
  let dispatchMock;
  ...
  it('should initialize without errors', () => {
    const {result} = renderHook(() =>
      useForgotPasswordProcess({email: '', accessCode: ''}),
    );
    expect(result.current.onResetPasswordClick).toBeInstanceOf(Function);
  });

  it('should display an error message if the email is invalid', () => {
    validateEmail.mockReturnValue(false);

    const invalidData = {email: 'invalidEmail', accessCode: '12345'};

    act(() => {
      isValidInput(invalidData);
    });

    expect(showErrorFlashMessage).toHaveBeenCalledWith(expect.any(String));
  });

  it('should display an error message if access code is empty', () => {
    validateEmail.mockReturnValue(true);
    isStringNullOrEmpty.mockReturnValue(true);

    const invalidData = {email: 'user@example.com', accessCode: ''};
  ...
  it('should call dispatch with correct actions when valid data is provided', async () => {
    validateEmail.mockReturnValue(true);
    isStringNullOrEmpty.mockReturnValue(false);

    const validData = {email: 'user@example.com', accessCode: 'company123'};
    ...
    expect(dispatchMock).toHaveBeenCalledWith(
      setUserDetailsForResetPassword(getFormatedEmailAndAccessCode(validData)),
    );
    expect(dispatchMock).toHaveBeenCalledWith(
      requestPasswordLink(getFormatedEmailAndAccessCode(validData)),
    );

    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      ASYNC_RESET_CREDENTIALS,
      JSON.stringify(getFormatedEmailAndAccessCode(validData)),
    );
  });

  it('should handle global error messages and call showErrorFlashMessage', () => {
    useSelector.mockImplementation(selector =>
      selector({
        global: {
          isError: true,
