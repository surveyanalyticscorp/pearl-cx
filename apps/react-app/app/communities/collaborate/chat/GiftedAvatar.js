...
export default class GiftedAvatar extends React.Component {
  ...
  renderAvatar() {
    if (typeof this.props.message.profilePic === 'function') {
      return this.props.message.profilePic();
    } else if (typeof this.props.message.profilePic === 'string') {
      return (
        <CachedImage
          source={{uri: this.props.message.profilePic}}
  ...
  renderInitials() {
    return (
      <CustomText style={[defaultStyles.textStyle, this.props.textStyle]}>
        {this.avatarName}
      </CustomText>
    );
  }
  ...
  render() {
    if (!this.props.message.username && !this.props.message.profilePic) {
      // render placeholder
      return (
        <View style={[
