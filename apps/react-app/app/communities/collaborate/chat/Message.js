...
export default class Message extends React.Component {
  ...
  isSameDay(currentMessage = {}, diffMessage = {}) {
    let diff = 0;
    if (diffMessage.timestamp && currentMessage.timestamp) {
      diff = Math.abs(moment(diffMessage.timestamp).startOf('day').diff(moment(currentMessage.timestamp).startOf('day'), 'days'));
    } else {
      diff = 1;
    }
    if (diff === 0) {
  ...
  renderBubble() {
    ...
    return <ChatMessageBubble {...bubbleProps} />;
  }

  renderAvatar() {
    if (this.props.useID !== this.props.currentMessage.memberID) {
      const {containerStyle, ...other} = this.props;
      const avatarProps = {
        ...other,
        isSameUser: this.isSameUser,
