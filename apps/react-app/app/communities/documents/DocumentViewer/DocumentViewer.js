...
class DocumentViewer extends BaseComponentWithoutScroll {
    ...
    componentDidMount() {
    this.handleDocumentReadStatus(this.props.document)
    }
    processAPIResponse(response) {
        console.log(response)
    }

    handleDocumentReadStatus(document) {
        if (document) {
            let addFavSurvey = document
            addFavSurvey.status = 2;
            this.props.updatePanelMemberDocuments(addFavSurvey).then(this.processAPIResponse);
        }
    }
    showSpinner() {
