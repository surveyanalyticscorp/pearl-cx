...
export default class MonthYearSelector extends Component {
    ...
    render() {
        ...
        return (
            <View style={[{padding: 20, backgroundColor:'white', minHeight:400}, this.props.style]}>
                {this.renderHeader()}
        ...
        return (
            <View style={{alignItems:'center', justifyContent:'center'}}>
                <TouchableHighlight
                    ...
                    this.props.onSubmit(this.state.selectedMonth, this.state.selectedYear)}
                }>
                    <View>
    ...
    renderHeader(){
        ...
        return (
            <View style={{flexDirection:'row', alignItems:'center'}}>
                <Icon color={secondaryColor} name={'date-range'} size={30} style={{marginRight:10}}/>
                <CustomText style={{color: primaryColor, fontSize:18, fontWeight:'bold'}}>{this.getMonthName(this.state.selectedMonth) + ", "
                                            + this.state.selectedYear}</CustomText>
            </View>
        )
    }
    ...
    renderPickerTitle(text) {
        ...
        return (
            <View style={styles.pickerTitle}>
                <CustomText style={{color:secondaryColor}}>{text}:</CustomText>
            </View>
        )
    }
    ...
    renderSeparator() {
        const {secondaryColor} = this.props;
        return (
            <View style={{height: 0.5, backgroundColor: secondaryColor, marginTop:5, flexDirection: 'row', alignSelf: 'stretch'}}/>
        )
    }
    ...
    renderMonthPickerElement(){
        ...
        return (
            <Picker style={{ width: 180, height: 180, marginBottom: 40 }}
