...
func getLocalizationValue(value:String) -> String {
    
     let currentLanguage = UserDefaults.standard.object(forKey: LCLCurrentLanguageKey) as? String 
    
    print(currentLanguage ?? "");
    
    guard let path = Bundle.main.url(forResource: "Locaizable", withExtension: "strings", subdirectory: nil, localization: currentLanguage) else {
        return ""
    }
    guard let dict = NSDictionary(contentsOf: path) else {
        return""
    }
    guard let translation = dict.value(forKey: value) as? String else {
        return""
    }
    return translation;
}
...
open class Localize: NSObject {
    ...
    open class func availableLanguages(_ excludeBase: Bool = false) -> [String] {
        ...
        if availableLanguages.contains("pt-BR") {
            let indexOfptBR = availableLanguages.index(of: "pt-BR")
            availableLanguages[indexOfptBR!] = "pt"
        }
        if availableLanguages.contains("pt-PT") {
            let indexOfptPT = availableLanguages.index(of: "pt-PT")
            availableLanguages[indexOfptPT!] = "pt"
        }
