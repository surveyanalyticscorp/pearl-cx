...
open class OAuth2Swift: OAuthSwift {
    ...
    fileprivate func requestOAuthAccessToken(withParameters parameters: OAuthSwift.Parameters, headers: OAuthSwift.Headers? = nil, success: @escaping TokenSuccessHandler, failure: FailureHandler?) -> OAuthSwiftRequestHandle? {
        let successHandler: OAuthSwiftHTTPRequest.SuccessHandler = { [unowned self]
            ...
            if let teamDict = responseParameters["team"] as? NSDictionary,
                let teamID = teamDict["id"] as? String  {
                self.client.credential.oauth_id = teamID
            }else{
                self.client.credential.oauth_id = ""
            }
            if let emailDict = responseParameters["user"] as? NSDictionary,
                let emailID = emailDict["email"] as? String  {
                self.client.credential.oauth_email = emailID
            }else{
                self.client.credential.oauth_email = ""
            }
