...
//            rootViewController = root.presentedViewController;
//        }
        ...
        self.alertController.popoverPresentationController.sourceView = rootViewController.view;
        self.alertController.popoverPresentationController.sourceRect = CGRectMake(rootViewController.view.bounds.size.width / 2.0, rootViewController.view.bounds.size.height, 1.0, 1.0);
...
- (void)actionHandler:(UIAlertAction *)action
{
    ...
    if (results.count > 0) {
        NSString *takePhotoButtonTitle = [[results objectAtIndex:0] objectForKey:@"takePhotoButtonTitle"];
        NSString *chooseFromLibraryButtonTitle = [[results objectAtIndex:0] objectForKey:@"chooseFromLibraryButtonTitle"];
        if ([action.title isEqualToString:takePhotoButtonTitle]) { // Take photo
            [self launchImagePicker:RNImagePickerTargetCamera];
        }
        else if ([action.title isEqualToString:chooseFromLibraryButtonTitle]) { // Choose from library
            [self launchImagePicker:RNImagePickerTargetLibrarySingleImage];
        }
        else {
            self.callback(@[@{@"customButton": [[results objectAtIndex:0] objectForKey:@"title"]}]);
            return;
        }
    }
