...
class ProgressDialog extends Component {
    render() {
        const {
            message, messageStyle, activityIndicatorColor, activityIndicatorSize, activityIndicatorStyle,isRTL
        } = this.props;

        return (
            <Dialog {...this.props} >
                <View style={[styles.loadingView, {flexDirection: isRTL ? 'row-reverse' : 'row'}]} >
                    <ActivityIndicator animating={true} color={activityIndicatorColor} size={activityIndicatorSize} style={activityIndicatorStyle} />
                    <Text style={[styles.defaultLoadingText, messageStyle, {textAlign: isRTL ? 'right' : 'left'}]}>{message}</Text>
                </View>
            </Dialog>
        )
    }
