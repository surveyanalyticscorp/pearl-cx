  ...
  {},
  {
    [types.CX_FEEDBACK_LIST](state, action) {
      if (action.data) {
        return {
          allResponses:
            action.requestData.pageOffset === 0
              ? action.data.body.allResponses
              : [...state.allResponses, ...action.data.body.allResponses],
          lastAddedCount: action.data.body.allResponses
            ? action.data.body.allResponses.length
            : 0,
        };
      }
